...
This requirement was introduces to attach timestamp marking time when value was initially read and allows you to add additional
informations as well, ie. Temperature can be passed with correct scale, etc.

Currently we provided following implementations for storing:

* temperature - `tigase.rpi.home.values.Temperature`
* light - `tigase.rpi.home.values.Light`
* movement - `tigase.rpi.home.values.Movement`
...
Constructor of an `AbstractSensor` class requires in a type of a device as a parameter - string value.
...
This class will allow you to specify how often to read from a sensor and implement actual read from a sensor inside `<T extends IValue> T readValue()` method.
...
Constructor of an `AbstractSensor` class requires in a type of a device as a parameter - string value and default period in which data from sensor will be read.
