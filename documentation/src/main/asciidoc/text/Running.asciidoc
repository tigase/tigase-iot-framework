...
PubSub service allows modifications of PubSub nodes only by node owners. Due to that it is required that every IoT-XMPP bridge is required to use authenticated connection to XMPP server.
...
[WARNING]
In current version of Tigase Raspberry Pi Home project it is possible to use ANONYMOUS authentication with properly configured XMPP server, but it is not recommended due to security considerations.
