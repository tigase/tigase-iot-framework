...
[WARNING]
Current version of Tigase Raspberry Pi Home project requires authenticated connection to XMPP server, which means it is not possible to use ANONYMOUS authentication.
Additionally evert IoT-XMPP bridge needs to use same credentials for authentication to XMPP server (must use same BareJID).
